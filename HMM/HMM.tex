...
$n$で添え字付けられた$\bm{x}_n$や$\bm{z}_n$と言った確率変数に関してすべて$\mathcal{F}_n$-可測であるとするという感じか}。
...
以下の議論で$\sum_{\bm{z}_n}$という記号が出てきた場合は、上述の$K$個の$0$と$1$からなるベクトルに対して和を走らせるものと定義しておく。


さらに隠れマルコフモデルにおいて潜在状態間の遷移を表す条件付き分布$p(\bm{z}_n|\bm{z}_{n-1})$を特徴づけるパラメーター、$K \times K$行列$A$を導入しよう。
この行列$A$の各成分は
\begin{align}
...
潜在変数の初期状態$\bm{z}_1$も同様に$K$次元ベクトルのパラメーター$\bm{pi}\in \mathbb{K}$を
\begin{align}
    \begin{aligned}
        \bm{\pi} &=
        \begin{pmatrix}
...
以降では、本章で導入したパラメーター$A,\bm{\pi},\bm{\phi}$をまとめて$\theta := \{\bm{\pi},A,\bm{\phi}\}$と書く事にしよう。
...
モデルのパラメーター$\theta=\{\bm{\pi},A,\bm{\phi}\}$を最尤推定により求めることを考える。
...
\begin{align} 
	...
	\label{}
\end{align}
この$\gamma(\bm{z}_n),\xi(\bm{z}_{n-1},\bm{z}_n)$について潜在変数$z$と同じく、各要素での表現
\begin{align}
    \begin{aligned}
...
\begin{aligned}
	...
	+  \sum_{n=2}^{N}  \sum_{\bm{z}_n,\bm{z}_{n-1}} \sum_{j=1}^{K} \sum_{k=1}^{K} \xi(\bm{z}_{n-1},\bm{z}_n) z_{n-1,j} z_{nk} \ln A_{jk} 
...
\begin{align}
	\begin{aligned}
		\theta_{max}    &=  \argmax_{\theta=(\pi,A,\phi)} F(\bm{\pi}, A, \bm{\phi}) \\ 
...
\end{align}
とすることが出来る。残りのパラメーター$\bm{\phi}$についての微分は観測変数$x$がどのような分布に従うと設定するかに応じて結果が変わるものであるので、ここでは示さない。
...
\begin{align}
    \begin{aligned}
	    \gamma(\bm{z}_n) &= p(\bm{z}_n|X) = \frac{p(X|\bm{z}_n)p(\bm{z}_n)}{p(X)} \\
...
$\gamma(\bm{z}_N)=p(\bm{z}_N|X)$,$\alpha(\bm{z}_N)=p(\bm{x}_1,\dots,\bm{x}_N,\bm{z}_N)$である一方、
\begin{align}
...
\end{align}
\begin{align}
	1 = \sum_{\bm{z}_n} \gamma(\bm{z}_n) = \sum_{\bm{z}_n} \frac{p(\alpha(\bm{z}_n)\beta(\bm{z}_n)}{p(X)} = \frac{1}{p(X)} \sum_{\bm{z}_n} p(\alpha(\bm{z}_n)\beta(\bm{z}_n) \\
	\therefore p(X) = \sum_{\bm{z}_n} p(\alpha(\bm{z}_n)\beta(\bm{z}_n)
	\label{}
\end{align}
として尤度関数$p(X)$を計算することが出来る。上式は任意の$n \in \left\{ 1,\cdots, N \right\}$に対して成立する事、
またで見たように$\beta(\bm{z}_N)=1$である事を勘案すると
\begin{align}
	p(X) = \sum_{\bm{z}_N} p(\alpha(\bm{z}_N))
	\label{}
\end{align}
と書くことが出来る。これでPRML\cite{Bishop}の(13.41)(13.42)を導出する事が出来た。


$\xi(\bm{z}_{n-1},\bm{z}_n)$を求めるには以下のように計算する。 
\begin{align}
	\begin{aligned}
		\xi(\bm{z}_{n-1},\bm{z}_n) 
		&= p(\bm{z}_{n-1},\bm{z}_{n}|X) \\
		&= \frac{p(X|\bm{z}_{n-1},\bm{z}_n)p(\bm{z}_{n-1},\bm{z}_n)}{p(X)} \\
		&= \frac{p(\bm{x}_1,\dots,\bm{x}_{n-1}|\bm{z}_{n-1})p(\bm{x}_n|\bm{z}_n)p(\bm{x}_{n+1},\dots,\bm{x}_N|\bm{z}_n)p(\bm{x}_{n+1},\dots,\bm{x}_N|\bm{z}_n)p(\bm{z}_n|\bm{z}_{n-1})p(\bm{z}_{n-1})}{p(X)}\\
		&= \frac{p(\bm{x}_1,\dots,\bm{x}_{n-1}|\bm{z}_{n-1})p(\bm{z}_{n-1})p(\bm{x}_n|\bm{z}_n)p(\bm{x}_{n+1},\dots,\bm{x}_N|\bm{z}_n)p(\bm{x}_{n+1},\dots,\bm{x}_N|\bm{z}_n)p(\bm{z}_n|\bm{z}_{n-1})}{p(X)}\\
		&= \frac{\alpha(\bm{z}_{n-1})p(\bm{x}_n|\bm{z}_n)p(\bm{x}_{n+1},\dots,\bm{x}_N|\bm{z}_n)p(\bm{x}_{n+1},\dots,\bm{x}_N|\bm{z}_n)\beta(\bm{z}_n)}{p(X)}
	\end{aligned}
	\label{}
\end{align}
...
\begin{enumerate}
    ...
    \item フォワード$\alpha$再帰、バックワード$\beta$再帰を実行し、$\gamma(\bm{z}_n),\xi(\bm{z}_{n-1},\bm{z}_n)$を求める（Eステップ完了）
